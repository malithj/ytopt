...
#                     cuda = True
    ...
    def findRuntime(self, x, params):
        ...
        commonflags = f"""-DDATA_TYPE_IS_FLOAT -DPOLYBENCH_STACK_ARRAYS -DEXTRALARGE_DATASET -DPOLYBENCH_TIME -I{utilities_dir} -I{kernel_dir} {interimfile} {utilities_dir}/polybench.c -o {tmpbinary} -lm -g """
        ...
        # print (run_cmd)
        run_cmd = 'CUDA_VISIBLE_DEVICES=0 ' + kernel_dir + "/exe.pl " + tmpbinary
        power_cmd =  "CUDA_VISIBLE_DEVICES=0 python " + kernel_dir + "/power.py " + tmpbinary
        ...
        if compilation_status.returncode == 0:
            ...
            power_run = subprocess.run(
                power_cmd, shell=True, stdout=subprocess.PIPE)
            power = float(power_run.stdout.decode('utf-8').rstrip('\n').strip(' '))
        else:
            print(compilation_status.stderr)
            print("compile failed")

        # return execution time as cost
        return exetime, power, interimfile.split('/')[-1]

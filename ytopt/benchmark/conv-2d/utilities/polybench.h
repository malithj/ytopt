#define cudaSafeCall(call) {	   \
  enum cudaError err = call;       \
  if(cudaSuccess != err){     \
    fprintf(stderr, "%s(%i) : %s.\n", __FILE__, __LINE__, cudaGetErrorString(err));   \
    exit(EXIT_FAILURE);       \
}}

//when calling cudaMallocManaged, we allocate device memory

# define POLYBENCH_ARRAY_CUDA(var) var

//2d array
# define POLYBENCH_2D_ARRAY_DECL_CUDA(var,type,ANI,ANJ,ni,nj) type *var=NULL; cudaSafeCall(cudaMallocManaged((void **) &var, (ANI)*(ANJ)*(sizeof(type)),1));
# define POLYBENCH_2D_CUDA(var, dim1, dim2, ddim1, ddim2) *var
# define POLYBENCH_2D_REF(var, dim1, dim2, i, j) var[(i)*(dim1)+(j)]
//&var is the host side pointer variable

//make one for 1d array
//# define POLYBENCH_1D_ARRAY_DECL_CUDA(var,type,ANI,ni) type *var=NULL; cudaSafeCall(cudaMallocManaged((void **) &var, (ANI)*(sizeof(type)),1));
//# define POLYBENCH_1D_CUDA(var, dim1, ddim1) *var
//# define POLYBENCH_1D_REF(var, dim1, i) var[i]

//make one for 3d array
# define POLYBENCH_3D_ARRAY_DECL_CUDA(var,type,ANI,ANJ,ANK,ni,nj,nk) type *var=NULL;cudaSafeCall(cudaMallocManaged((void **) &var, (ANI)*(ANJ)*(ANK)*(sizeof(type)),1));
# define POLYBENCH_3D_CUDA(var, dim1, dim2, dim3, ddim1, ddim2, ddim3) *var
# define POLYBENCH_3D_REF(var, dim1, dim2, dim3, i, j, k) var[(i)*(dim1)*(dim2)+(j)*(dim2)+(k)]

//clean up memory
//#  define POLYBENCH_FREE_ARRAY(x) free(x);
